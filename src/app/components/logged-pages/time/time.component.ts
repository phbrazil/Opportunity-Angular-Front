...
export class TimeComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.timeService.getCurrentDay().subscribe((res) => {
      if (res != null) {
        this.day = res;
        this.currentDayFormatted = moment(new Date(this.day)).format(
          'Do MMMM YYYY'
        );
  ...
  selectToday() {
    ...
    this.currentDayFormatted = moment(
  ...
  loadTasks() {
    ...
    this.timeService
      .getEntries(
        this.user.idUser,
        moment(this.day).format('DD-MM-YYYY'),
        ...
        (res) => {
          ...
          this.tasks = res;
           ...
           res.forEach((element) => {
            this.total =
              this.total +
              this.timeService.calcTime(Number(element.time.replace(':', '.')));
          });
