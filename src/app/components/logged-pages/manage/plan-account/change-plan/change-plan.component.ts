import { ChangeDetectorRef, Component, EventEmitter, Inject, LOCALE_ID, OnInit, Output, ViewEncapsulation } from '@angular/core';
...
export class ChangePlanComponent implements OnInit {
  ...
  @Output() notifyPlanAccount: EventEmitter<boolean> = new EventEmitter();
  ...
  onSubmit() {
    ...
    this.planService.changePlan(this.plan, this.plan.idPlan, this.accountService.getToken()).subscribe(res => {
      ...
      this.notifyPlanAccount.emit(true);
    }, _err => {
      this.isLoading = false;
    })
  }
